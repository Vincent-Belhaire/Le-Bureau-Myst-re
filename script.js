(() => {
  ...
  const helpCopy = {
    general: `<p>Dans chaque défi, lis d’abord la consigne placée au-dessus de l’écran.</p><ul><li>Essaie une action à la fois.</li><li>Le message sous l’activité te dit tout de suite si tu avances.</li><li>Le bouton « Coup de pouce » donne un indice sans retirer de point.</li></ul>`,
  ...
  function createQuiz({ arena, count, context, question, answers, feedback, items, keyPrefix, noun, onComplete }) {
    let index = 0;
  ...
  const startZoneQuiz = createQuiz({
    ...
    feedback: zoneFeedback, items: zoneQuizItems, keyPrefix: "zone-quiz", noun: "Question",
    onComplete: () => {
  ...
  document.getElementById("folderLauncher").addEventListener("click", () => {
    if (state.windowIndex === 0) setFeedback(windowFeedback, "C’était un clic simple. Essaie deux clics rapides, sans bouger la souris.", "error");
  });
  ...
  folderWindow.querySelector("[data-window-action='close']").addEventListener("click", () => {
    if (state.windowIndex < 6) setFeedback(windowFeedback, "Attention : ce bouton fermerait le dossier. Suis la consigne affichée.", "error");
  });
  ...
  const startWindowQuiz = createQuiz({
    ...
    feedback: windowFeedback, items: windowQuizItems, keyPrefix: "window-quiz", noun: "Incident",
    onComplete: () => {
  ...
  document.getElementById("doubleTarget").addEventListener("dblclick", event => { event.currentTarget.classList.add("open"); advanceMouse(1, "Double-clic réussi : le coffre est ouvert."); });
  document.getElementById("doubleTarget").addEventListener("click", () => { if (state.mouseIndex === 1) setFeedback(mouseFeedback, "Encore ! Deux clics très rapprochés ouvrent le coffre.", "error"); });
  ...
  document.getElementById("rightTarget").addEventListener("click", () => { if (state.mouseIndex === 2) setFeedback(mouseFeedback, "Ce fichier attend un clic avec le bouton droit de la souris.", "error"); });
  ...
  const fallbackDrop = () => { if (fileSelected && state.mouseIndex === 3) { dragFile.hidden = true; advanceMouse(3, "Fichier rangé ! C’était la méthode de secours sans glissement."); } };
  ...
  const startMouseQuiz = createQuiz({
    ...
    feedback: mouseFeedback, items: mouseQuizItems, keyPrefix: "mouse-quiz", noun: "Situation",
    onComplete: () => {
  ...
  sourceText.addEventListener("keydown", event => {
    ...
    if (key === "a" && state.keyboardIndex === 0) {
      ...
      else setFeedback(keyboardFeedback, "Sélectionne d’abord tout le message avec Ctrl + A.", "error");
    }
  });
  ...
  targetText.addEventListener("keydown", event => {
    if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "v" && state.keyboardIndex === 2) {
      ...
      else setFeedback(keyboardFeedback, "Rien n’a encore été copié. Reprends les étapes dans l’ordre.", "error");
    }
  });
  ...
  function showFinal() {
    ...
    const stars = state.score >= 3600 ? 3 : state.score >= 3000 ? 2 : 1;
    document.getElementById("starRow").textContent = `${"★ ".repeat(stars)}${"☆ ".repeat(3 - stars)}`.trim();
    const rank = stars === 3 ? "Maître du bureau" : stars === 2 ? "Pilote numérique" : "Explorateur du bureau";
    document.getElementById("rankBadge").textContent = rank;
    document.getElementById("keyboardSkill").classList.toggle("locked", !state.bonusComplete);
    document.getElementById("keyboardSkill").querySelector("span").textContent = state.bonusComplete ? "✓" : "○";
    const labels = [["Repérage", 1000], ["Fenêtres", 1000], ["Souris", 1000], ["Clavier", 1000]];
    document.getElementById("resultDetails").innerHTML = labels.map(([label, max], index) => {
      const prefixes = ["zone-", "window-", "mouse-", "keyboard-"];
      const earned = [...state.awarded].filter(key => key.startsWith(prefixes[index])).reduce((sum, key) => {
        if (key.startsWith("keyboard-typing-")) return sum + 300;
        return sum + (key === "keyboard-2" ? 200 : 100);
      }, 0);
      return `<div><strong>${earned} / ${max}</strong><small>${label}</small></div>`;
    }).join("");
  }
